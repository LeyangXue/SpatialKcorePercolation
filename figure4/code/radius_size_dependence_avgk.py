...
if __name__ == "__main__":
    ...
    avg_ks = [round(each,2) for each in np.arange(7.0, 7.42, 0.1)]
    simulation = 10
    zetas = [18] #characteristic length 
    ...
    for avg_k in avg_ks:
         ...
         networkpath_avgk = networkpath+f'/size_dependency/L{L}/avgk'+str(avg_k)
         ...
         for kcore in kcores:
             ...
             Rh_high = L/2
             threshold = 0.00001
             RhcParallel(savepath, networkpath, zetas, avg_k, kcore, Rh_low, Rh_high, threshold, simulation)
